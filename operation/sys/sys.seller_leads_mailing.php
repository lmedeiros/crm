<?php
    ...
    function getRecords() {
	$accounts = $GLOBALS['db']->selectAll("SELECT L.* FROM sys_mailing_lead L WHERE sys_seller_id='" . $_GET['user_id'] . "' AND sys_mailing_id = '" . @$_GET['id'] . "' AND " . query_filter());
        if($accounts) {
            return $accounts;
        } else {
            return false;
...
//    debug($titles); 
    ...
    $columns = array("field1_value", "field2_value", "field3_value", "field4_value", "field5_value", "field6_value", "field7_value", "field8_value", "is_worked","ACTIONS");
    #$actions array of operations ID related to the page type LIST
    $actions = array(117, 97, 108);
    $actions_confirm = Array(97=>'Are you sure to delete?');
    ...
    $table->addFilter(8, "filterBoolean");
    ...
    #show tables
	    new CreateHome(@$_GET['operation'], array(0),array('id='.$_GET['user_id']=>106));
    $table->displayTable();
?>
