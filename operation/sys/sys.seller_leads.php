<?php
    ...
    function getRecords() {
	$accounts = $GLOBALS['db']->selectAll("SELECT DISTINCT M.* FROM sys_mailing M INNER JOIN sys_mailing_lead L ON L.sys_seller_id = '" . $_GET['id'] . "' AND L.sys_mailing_id = M.sys_mailing_id AND " . query_filter());
        if($accounts) {
            return $accounts;
        } else {
            return false;
	...
	$accounts = $GLOBALS['db']->selectRow("SELECT count(L.sys_mailing_id) as tot FROM sys_mailing_lead L WHERE L.sys_mailing_id='{$value}' AND L.sys_seller_id='" . $_GET['id'] . "'");
	       if($accounts) {
            return $accounts['tot'];
        } else {
            return false;
   ...
   function getWorked($value) {
        $accounts = $GLOBALS['db']->selectRow("SELECT count(L.sys_mailing_id) as tot FROM sys_mailing_lead L WHERE L.sys_mailing_id='{$value}' AND L.is_worked=1 AND L.sys_seller_id='" . $_GET['id'] . "'");
               if($accounts) {
            return $accounts['tot'];
        } else {
            return false;
    ...
    #show tables
    new CreateHome(@$_GET['operation'], array(0), 107);
    $table->displayTable();
?>
