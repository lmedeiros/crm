<?php
    ...
    function getRecords() {
	$accounts = $GLOBALS['db']->selectAll("SELECT M.* FROM sys_mailing M WHERE " . query_filter());
        if($accounts) {
            return $accounts;
        } else {
            return false;
	...
	$accounts = $GLOBALS['db']->selectRow("SELECT count(L.sys_mailing_id) as tot FROM sys_mailing_lead L WHERE L.sys_mailing_id='{$value}' AND L.sys_seller_id=0");
	       if($accounts) {
            return $accounts['tot'];
        } else {
            return false;
     ...
     function getAll($value) {
       ...
       function getWorked($value) {
        $accounts = $GLOBALS['db']->selectRow("SELECT count(L.sys_mailing_id) as tot FROM sys_mailing_lead L WHERE L.sys_mailing_id='{$value}' AND L.is_worked=1 ");
               if($accounts) {
            return $accounts['tot'];
        } else {
            return false;
    ...
    $headers = array("Description", "Filename", "Loaded on" , "Free Leads" , "Worked Leads", "Total Leads","Actions");
    ...
    $actions = array(99, 113, 95, 103, 118);
    ...
    #show tables
    new CreateHome(@$_GET['operation'], array(100),0);
    $table->displayTable();
?>
