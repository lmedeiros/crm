<?php
    ...
    function getRecords() {
	$accounts = $GLOBALS['db']->selectAll("SELECT L.* FROM sys_mailing_lead L WHERE sys_seller_id='" . $_SESSION[session_key] . "' AND sys_mailing_id = '" . @$_GET['id'] . "' AND " . query_filter());
        if($accounts) {
            return $accounts;
        } else {
            return false;
...
//    debug($titles); 
    ...
    #show tables
	    new CreateHome(@$_GET['operation'], array(0),109);
    $table->displayTable();
?>
