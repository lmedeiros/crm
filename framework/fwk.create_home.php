...
class CreateHome {
	...
	if ($back_operation) {
		if(is_array($back_operation_id)) {
		        $this->write_icon($back_operation, key($back_operation_id), true);
		} else {
    ...
    public function write_icon($operation,$param, $back=false) {
        ...
        " onmouseover=" . "'this.style.background=\"#FEF0DB\"' " .
        " onmouseout=" . "'this.style.background=\"#fefefe\"'>";
        echo "<p class='home_icon'>\n";
        echo "<a title='{$operation['description']}' href='?operation={$operation['fwk_operation_id']}&amp;{$param}'>\n";
        if ($back == false) {
        ...
        echo "</p>\n";
        echo "<p class='home_icon'>\n";
        echo "<a  title='{$operation['description']}' href='?operation={$operation['fwk_operation_id']}&amp;{$param}'>" . ($operation['name']) . "</a>\n";
        echo "</p>\n";
        echo "</div>\n";
    }
